...
| **PASSWD**<br>`$ sudo passwd user` | Sets or changes a user's password. | 1. Set pw: `sudo passwd testuser`<br>2. Expire pw: `sudo passwd -e testuser`<br>3. Lock/unlock: `sudo passwd -l testuser` (lock)<br>4.Lock/unlock status: `sudo passwd -S <username>` |
